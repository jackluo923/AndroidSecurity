  ...
  extends Draft
{
    ...
    throws InvalidDataException
  {
      ...
      else
      {
        if (b == -1)
        {
          ...
          if (currentFrame != null)
          {
            ...
            localFramedataImpl1.setPayload(currentFrame);
            localFramedataImpl1.setFin(true);
            if (!inframe) {
              break label138;
            }
          ...
          label138:
          for (localObject = Framedata.Opcode.CONTINUOUS;; localObject = Framedata.Opcode.TEXT)
          {
            localFramedataImpl1.setOptcode((Framedata.Opcode)localObject);
            readyframes.add(localFramedataImpl1);
            currentFrame = null;
    ...
    if (readingState)
    {
      ...
      currentFrame.flip();
      ((FramedataImpl1)localObject).setPayload(currentFrame);
    ...
    label297:
    for (paramByteBuffer = Framedata.Opcode.CONTINUOUS;; paramByteBuffer = Framedata.Opcode.TEXT)
    {
      ((FramedataImpl1)localObject).setOptcode(paramByteBuffer);
      inframe = true;
      readyframes.add(localObject);
      paramByteBuffer = readyframes;
