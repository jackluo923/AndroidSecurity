  ...
  extends Draft_75
{
  ...
  public ClientHandshakeBuilder postProcessHandshakeRequestAsClient(ClientHandshakeBuilder paramClientHandshakeBuilder)
  {
    ...
    if (!paramClientHandshakeBuilder.hasFieldValue("Origin")) {
      paramClientHandshakeBuilder.put("Origin", "random" + reuseableRandom.nextInt());
    }
    ...
    throws InvalidDataException
  {
    ...
    if (localList == null)
    {
      ...
      localList = readyframes;
      readingState = true;
      if (currentFrame == null)
      {
        currentFrame = ByteBuffer.allocate(2);
        if (paramByteBuffer.remaining() > currentFrame.remaining()) {
      ...
      currentFrame.put(paramByteBuffer);
  ...
  public Handshakedata translateHandshake(ByteBuffer paramByteBuffer)
    throws InvalidHandshakeException
  {
    HandshakeBuilder localHandshakeBuilder = translateHandshakeHttp(paramByteBuffer, role);
    if (((localHandshakeBuilder.hasFieldValue("Sec-WebSocket-Key1")) || (role == WebSocket.Role.CLIENT)) && (!localHandshakeBuilder.hasFieldValue("Sec-WebSocket-Version"))) {
...
/* Location:
 * Qualified Name:     org.java_websocket.drafts.Draft_76
