...
public final class Version
{
  ...
  private Version(int paramInt, int[] paramArrayOfInt, ECBlocks... paramVarArgs)
  {
    ...
    alignmentPatternCenters = paramArrayOfInt;
  ...
  BitMatrix buildFunctionPattern()
  {
    ...
    int m = alignmentPatternCenters.length;
    int i = 0;
    while (i < m)
    {
      int n = alignmentPatternCenters[i];
      int j = 0;
      if (j < m)
      {
        ...
        for (;;)
        {
          j += 1;
          break;
          localBitMatrix.setRegion(alignmentPatternCenters[j] - 2, n - 2, 5, 5);
        }
      }
      i += 1;
    }
  ...
  {
    return ecBlocks[paramErrorCorrectionLevel.ordinal()];
  }
