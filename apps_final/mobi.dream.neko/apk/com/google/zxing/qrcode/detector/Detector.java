...
public class Detector
{
  ...
  private float sizeOfBlackWhiteBlackRunBothWays(int paramInt1, int paramInt2, int paramInt3, int paramInt4)
  {
    ...
    for (;;)
    {
      ...
      f1 = (image.getWidth() - 1 - paramInt1) / (i - paramInt1);
      paramInt3 = image.getWidth() - 1;
      break;
      label146:
      paramInt4 = i;
      if (i >= image.getHeight())
      {
        f1 = (image.getHeight() - 1 - paramInt2) / (i - paramInt2);
        paramInt4 = image.getHeight() - 1;
      }
    }
  }
    ...
    throws NotFoundException, FormatException
  {
    ...
    for (ResultPointCallback localResultPointCallback = null;; localResultPointCallback = (ResultPointCallback)paramMap.get(DecodeHintType.NEED_RESULT_POINT_CALLBACK))
    {
      ...
      return processFinderPatternInfo(new FinderPatternFinder(image, resultPointCallback).find(paramMap));
    }
  }
    ...
    throws NotFoundException
  {
    ...
    paramInt1 = Math.min(image.getWidth() - 1, paramInt1 + j);
    if (paramInt1 - i < paramFloat1 * 3.0F) {
      throw NotFoundException.getNotFoundInstance();
    }
    int k = Math.max(0, paramInt2 - j);
    paramInt2 = Math.min(image.getHeight() - 1, paramInt2 + j);
    if (paramInt2 - k < paramFloat1 * 3.0F) {
      throw NotFoundException.getNotFoundInstance();
    }
    return new AlignmentPatternFinder(image, i, k, paramInt1 - i, paramInt2 - k, paramFloat1, resultPointCallback).find();
  }
    ...
    throws NotFoundException, FormatException
  {
    ...
    for (;;)
    {
      try
      {
        ...
        localObject2 = sampleGrid(image, (PerspectiveTransform)localObject1, j);
