...
public final class ErrorCorrection
{
  ...
  private int[] findErrorMagnitudes(ModulusPoly paramModulusPoly1, ModulusPoly paramModulusPoly2, int[] paramArrayOfInt)
  {
    ...
    while (i <= j)
    {
      arrayOfInt[(j - i)] = field.multiply(i, paramModulusPoly2.getCoefficient(i));
      i += 1;
    }
    paramModulusPoly2 = new ModulusPoly(field, arrayOfInt);
    j = paramArrayOfInt.length;
    arrayOfInt = new int[j];
    i = 0;
    while (i < j)
    {
      int m = field.inverse(paramArrayOfInt[i]);
      int k = field.subtract(0, paramModulusPoly1.evaluateAt(m));
      m = field.inverse(paramModulusPoly2.evaluateAt(m));
      arrayOfInt[i] = field.multiply(k, m);
      i += 1;
    }
    return arrayOfInt;
  }
    ...
    throws ChecksumException
  {
    ...
    localModulusPoly2 = field.getZero();
    paramModulusPoly1 = field.getOne();
    ...
    for (;;)
    {
      ...
      paramModulusPoly2 = localModulusPoly3;
      paramModulusPoly1 = field.getZero();
      ...
      while ((paramModulusPoly2.getDegree() >= localModulusPoly1.getDegree()) && (!paramModulusPoly2.isZero()))
      {
        ...
        int k = field.multiply(paramModulusPoly2.getCoefficient(paramModulusPoly2.getDegree()), i);
        paramModulusPoly1 = paramModulusPoly1.add(field.buildMonomial(j, k));
  ...
  public void decode(int[] paramArrayOfInt1, int paramInt, int[] paramArrayOfInt2)
    throws ChecksumException
  {
    Object localObject = new ModulusPoly(field, paramArrayOfInt1);
    ...
    while (i > 0)
    {
      k = ((ModulusPoly)localObject).evaluateAt(field.exp(i));
    ...
    if (j != 0)
    {
      localObject = field.getOne();
      j = paramArrayOfInt2.length;
      i = 0;
      while (i < j)
      {
        k = paramArrayOfInt2[i];
        k = field.exp(paramArrayOfInt1.length - 1 - k);
        localObject = ((ModulusPoly)localObject).multiply(new ModulusPoly(field, new int[] { field.subtract(0, k), 1 }));
        i += 1;
      }
      paramArrayOfInt2 = new ModulusPoly(field, arrayOfInt);
      localObject = runEuclideanAlgorithm(field.buildMonomial(paramInt, 1), paramArrayOfInt2, paramInt);
      ...
      {
        i = paramArrayOfInt1.length - 1 - field.log(localObject[paramInt]);
        if (i < 0) {
          throw ChecksumException.getChecksumInstance();
        }
        paramArrayOfInt1[i] = field.subtract(paramArrayOfInt1[i], paramArrayOfInt2[paramInt]);
        paramInt += 1;
      }
    }
  }
