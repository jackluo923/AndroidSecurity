  ...
  extends Binarizer
{
  ...
  {
    if (luminances.length < paramInt) {
      luminances = new byte[paramInt];
    }
    paramInt = 0;
    while (paramInt < 32)
    {
    ...
    throws NotFoundException
  {
    ...
    initArrays(k);
    int[] arrayOfInt = buckets;
    int i = 1;
    int j;
    while (i < 5)
    {
      byte[] arrayOfByte = ((LuminanceSource)localObject).getRow(m * i / 5, luminances);
      n = (k << 2) / 5;
      j = k / 5;
      while (j < n)
      {
    ...
    throws NotFoundException
  {
    ...
    {
      initArrays(m);
      localObject = ((LuminanceSource)localObject).getRow(paramInt, luminances);
      arrayOfInt = buckets;
      paramInt = 0;
      while (paramInt < m)
      {
