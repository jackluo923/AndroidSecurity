...
public class ImageProcessing
{
  ...
  public static Bitmap getCompressedBitmap(String paramString, Context paramContext)
  {
    try
    {
      ...
      paramContext.close();
      i = 1;
      while (outWidth * outHeight * (1.0D / Math.pow(i, 2.0D)) > j) {
        i += 1;
      }
      paramContext = new FileInputStream(paramString);
      if (i > 1)
      ...
      {
        paramContext.close();
        Log.debug("[InMobi]-[RE]-4.4.1", "bitmap size - width: " + paramString.getWidth() + ", height: " + paramString.getHeight());
        return paramString;
    ...
    {
      Log.debug("[InMobi]-[RE]-4.4.1", paramString.getMessage(), paramString);
    }
  }
