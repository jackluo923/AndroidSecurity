  ...
  extends BaseMappedStatement<T, ID>
{
  ...
  private static <T, ID> int updateRows(DatabaseConnection paramDatabaseConnection, Class<T> paramClass, MappedDeleteCollection<T, ID> paramMappedDeleteCollection, Object[] paramArrayOfObject, ObjectCache paramObjectCache)
  {
    try
    {
      int j = paramDatabaseConnection.delete(statement, paramArrayOfObject, argFieldTypes);
      ...
      logger.debug("delete-collection with statement '{}' and {} args, changed {} rows", statement, Integer.valueOf(paramArrayOfObject.length), Integer.valueOf(j));
    ...
    {
      throw SqlExceptionUtil.create("Unable to run delete collection stmt: " + statement, paramDatabaseConnection);
    }
  }
}
