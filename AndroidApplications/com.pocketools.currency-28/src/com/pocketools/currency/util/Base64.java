package com.pocketools.currency.util;
...
public class Base64 {
    ...
    static {
        $assertionsDisabled = Base64.class.desiredAssertionStatus() ? DECODE : true;
    ...
    public static byte[] decode(byte[] source, int off, int len, byte[] decodabet) throws Base64DecoderException {
        ...
        while (i < len) {
            ...
            if (sbiDecode >= (byte) -5) {
                if (sbiDecode < (byte) -1) {
                    b4Posn2 = b4Posn;
                } else if (sbiCrop == (byte) 61) {
                    int bytesLeft = len - i;
                    byte lastByte = (byte) (source[(len - 1) + off] & 127);
                    if (b4Posn == 0 || b4Posn == 1) {
                        throw new Base64DecoderException(new StringBuilder("invalid padding byte '=' at byte offset ").append(i).toString());
                    } else if ((b4Posn != 3 || bytesLeft <= 2) && (b4Posn != 4 || bytesLeft <= 1)) {
                        if (!(lastByte == (byte) 61 || lastByte == (byte) 10)) {
                            throw new Base64DecoderException("encoded value has invalid trailing byte");
                        }
                        if (b4Posn != 0) {
                        } else if (b4Posn != 1) {
                            throw new Base64DecoderException(new StringBuilder("single trailing character at offset ").append(len - 1).toString());
                        } else {
                            b4Posn2 = b4Posn + 1;
                            b4[b4Posn] = (byte) 61;
                        ...
                        throw new Base64DecoderException(new StringBuilder("padding byte '=' falsely signals end of encoded value at offset ").append(i).toString());
                    }
                } else {
                    b4Posn2 = b4Posn + 1;
                    b4[b4Posn] = sbiCrop;
                ...
                throw new Base64DecoderException(new StringBuilder("Bad Base64 input character at ").append(i).append(": ").append(source[i + off]).append("(decimal)").toString());
            }
        }
        if (b4Posn != 0) {
        } else if (b4Posn != 1) {
    ...
    private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, byte[] decodabet) {
        if (source[srcOffset + 2] == (byte) 61) {
            ...
            return R.styleable.com_admob_android_ads_AdView_textColor;
        } else {
    ...
    public static String encode(byte[] source, int off, int len, byte[] alphabet, boolean doPadding) {
        ...
        while (!doPadding && outLen > 0 && outBuff[outLen - 1] == (byte) 61) {
            outLen--;
        }
