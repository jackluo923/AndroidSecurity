package com.millennialmedia.android;

import android.content.Context;
import android.content.SharedPreferences.Editor;
import android.net.Uri;
import android.text.TextUtils;
import com.inmobi.commons.analytics.iat.impl.AdTrackerConstants;
import java.io.ByteArrayInputStream;
import java.io.File;
import java.io.IOException;
import java.lang.ref.WeakReference;
import java.net.MalformedURLException;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
import org.apache.http.HttpResponse;
import org.apache.http.client.methods.HttpGet;
import org.apache.http.impl.client.DefaultHttpClient;

class MRaid {
    static final String a = "loading";
    static final String b = "default";
    static final String c = "expanded";
    static final String d = "hidden";
    static final String e = "mraid.js";
    static final String f = "<script src=\"mraid.js\"></script>";
    static WeakReference g = null;
    private static final String h = "KEY_MMJS_URL";
    private static final String i = "<script.*src=[\"|']mraid\\.js[\"|']";
    private static final String j = "<head>";
    private static final String k = "MRaid";
    private static final String l = "http://cdn.millennialmedia.com/mmjs/v1.7/mm.js";
    private static final String m = "";
    private static final String n = "";

    final class AnonymousClass_1 implements Runnable {
        final /* synthetic */ String a;
        final /* synthetic */ Context b;

        AnonymousClass_1(String str, Context context) {
            this.a = str;
            this.b = context;
        }

        public void run() {
            Finished finished;
            try {
                MMLog.d(k, "MMJS -  download start (" + this.a + ")");
                HttpResponse execute = new DefaultHttpClient().execute(new HttpGet(this.a));
                MMLog.d(k, "MMJS -  download finish (" + this.a + ")");
                if (MRaid.a(this.b, execute.getEntity().getContent()) && MRaid.c(this.b, this.a)) {
                    MMLog.d(k, "MMJS -  download saved (" + this.a + ")");
                }
                if (g != null) {
                    finished = (Finished) g.get();
                    if (finished == null) {
                        return;
                    }
                    finished.finished();
                }
            } catch (MalformedURLException e) {
                MMLog.e(k, "Mraid download exception: ", e);
                if (g != null) {
                    finished = (Finished) g.get();
                    if (finished == null) {
                    }
                }
            } catch (IllegalStateException e2) {
                MMLog.e(k, "Mraid download exception: ", e2);
                if (g != null) {
                    finished = (Finished) g.get();
                    if (finished == null) {
                    }
                }
            } catch (IOException e3) {
                MMLog.e(k, "Mraid download exception: ", e3);
                if (g != null) {
                    finished = (Finished) g.get();
                    if (finished == null) {
                    }
                }
            }
        }
    }

    static interface Finished {
        void finished();
    }

    MRaid() {
    }

    static String a(Context context) {
        return Uri.fromFile(e(context)).toString();
    }

    static String a(Context context, String str) {
        Matcher matcher = Pattern.compile(i).matcher(str);
        String a = a(context);
        MMLog.b(k, "Mraid path: " + a(context));
        if (matcher.find()) {
            str = matcher.replaceFirst("<script src=\"" + a + "\"");
        } else if (str.indexOf(j) == -1) {
            str = "<script src=\"" + a + "\"></script>" + str;
        } else {
            matcher = Pattern.compile(j).matcher(str);
            if (matcher.find()) {
                str = matcher.replaceFirst("<head><script src=\"" + a + "\"></script>");
            }
        }
        return str.indexOf("<html>") == -1 ? "<html><head></head><body style='margin:0;padding:0;'>" + str + "</body></html>" : str;
    }

    static void a(Finished finished) {
        g = new WeakReference(finished);
    }

    /* JADX WARNING: inconsistent code. */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    static boolean a(android.content.Context r7, java.io.InputStream r8) {
        throw new UnsupportedOperationException("Method not decompiled: com.millennialmedia.android.MRaid.a(android.content.Context, java.io.InputStream):boolean");
        /*
        r0 = 1;
        r1 = 0;
        r4 = new java.io.File;
        r2 = com.millennialmedia.android.AdCache.h(r7);
        r3 = "mraid.js";
        r4.<init>(r2, r3);
        r2 = "MRaid";
        r3 = "Saving mraid js to %s";
        r5 = new java.lang.Object[r0];
        r5[r1] = r4;
        r3 = java.lang.String.format(r3, r5);
        com.millennialmedia.android.MMLog.a(r2, r3);
        r3 = 0;
        r2 = new java.io.FileOutputStream;	 Catch:{ Exception -> 0x0070, all -> 0x004b }
        r2.<init>(r4);	 Catch:{ Exception -> 0x0070, all -> 0x004b }
        r3 = 1024; // 0x400 float:1.435E-42 double:5.06E-321;
        r3 = new byte[r3];	 Catch:{ Exception -> 0x0037 }
    L_0x0026:
        r5 = r8.read(r3);	 Catch:{ Exception -> 0x0037 }
        if (r5 > 0) goto L_0x0032;
    L_0x002c:
        if (r2 == 0) goto L_0x0031;
    L_0x002e:
        r2.close();	 Catch:{ IOException -> 0x0065 }
    L_0x0031:
        return r0;
    L_0x0032:
        r6 = 0;
        r2.write(r3, r6, r5);	 Catch:{ Exception -> 0x0037 }
        goto L_0x0026;
    L_0x0037:
        r0 = move-exception;
    L_0x0038:
        if (r4 == 0) goto L_0x003d;
    L_0x003a:
        r4.delete();	 Catch:{ all -> 0x006e }
    L_0x003d:
        r3 = "MRaid";
        r4 = "Exception downloading component mraid.js: ";
        com.millennialmedia.android.MMLog.e(r3, r4, r0);	 Catch:{ all -> 0x006e }
        if (r2 == 0) goto L_0x0049;
    L_0x0046:
        r2.close();	 Catch:{ IOException -> 0x005c }
    L_0x0049:
        r0 = r1;
        goto L_0x0031;
    L_0x004b:
        r0 = move-exception;
        r2 = r3;
    L_0x004d:
        if (r2 == 0) goto L_0x0052;
    L_0x004f:
        r2.close();	 Catch:{ IOException -> 0x0053 }
    L_0x0052:
        throw r0;
    L_0x0053:
        r1 = move-exception;
        r2 = "MRaid";
        r3 = "Exception closing file: ";
        com.millennialmedia.android.MMLog.e(r2, r3, r1);
        goto L_0x0052;
    L_0x005c:
        r0 = move-exception;
        r2 = "MRaid";
        r3 = "Exception closing file: ";
        com.millennialmedia.android.MMLog.e(r2, r3, r0);
        goto L_0x0049;
    L_0x0065:
        r1 = move-exception;
        r2 = "MRaid";
        r3 = "Exception closing file: ";
        com.millennialmedia.android.MMLog.e(r2, r3, r1);
        goto L_0x0031;
    L_0x006e:
        r0 = move-exception;
        goto L_0x004d;
    L_0x0070:
        r0 = move-exception;
        r2 = r3;
        goto L_0x0038;
        */
    }

    static boolean a(String str) {
        return str == null ? false : Pattern.compile(i).matcher(str).find();
    }

    /* JADX WARNING: inconsistent code. */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    static java.lang.String b(android.content.Context r11) {
        throw new UnsupportedOperationException("Method not decompiled: com.millennialmedia.android.MRaid.b(android.content.Context):java.lang.String");
        /*
        r0 = d(r11);
        if (r0 != 0) goto L_0x0009;
    L_0x0006:
        r0 = "No MMJS Downloaded!";
    L_0x0008:
        return r0;
    L_0x0009:
        r6 = e(r11);
        r7 = 0;
        r3 = 0;
        r2 = "";
        r4 = "";
        r1 = "";
        r5 = new java.io.FileInputStream;	 Catch:{ Exception -> 0x00f2, all -> 0x0117 }
        r5.<init>(r6);	 Catch:{ Exception -> 0x00f2, all -> 0x0117 }
        r8 = r6.length();	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r0 = (int) r8;	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r0 = new byte[r0];	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r5.read(r0);	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r3 = "MD5";
        r3 = java.security.MessageDigest.getInstance(r3);	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r0 = r3.digest(r0);	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r2 = com.millennialmedia.android.MMSDK.a(r0);	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r3 = e(r11);	 Catch:{ Exception -> 0x0162, all -> 0x015d }
        r6 = new java.io.BufferedReader;	 Catch:{ Exception -> 0x016a, all -> 0x015d }
        r0 = new java.io.FileReader;	 Catch:{ Exception -> 0x016a, all -> 0x015d }
        r0.<init>(r3);	 Catch:{ Exception -> 0x016a, all -> 0x015d }
        r6.<init>(r0);	 Catch:{ Exception -> 0x016a, all -> 0x015d }
        r0 = 0;
        r7 = r0;
        r0 = r1;
        r1 = r4;
    L_0x0044:
        r4 = 10;
        if (r7 >= r4) goto L_0x004e;
    L_0x0048:
        r4 = r6.readLine();	 Catch:{ Exception -> 0x0176 }
        if (r4 != 0) goto L_0x008d;
    L_0x004e:
        if (r5 == 0) goto L_0x0053;
    L_0x0050:
        r5.close();	 Catch:{ Exception -> 0x0157 }
    L_0x0053:
        if (r6 == 0) goto L_0x0058;
    L_0x0055:
        r6.close();	 Catch:{ Exception -> 0x0157 }
    L_0x0058:
        r4 = new java.util.Date;
        r5 = r3.lastModified();
        r4.<init>(r5);
        r5 = android.text.TextUtils.isEmpty(r0);
        if (r5 == 0) goto L_0x0125;
    L_0x0067:
        r0 = "MMJS Stats: buildDate[%s] MD5[%s] dlDate[%s] Size[%d] Url[%s]";
        r5 = 5;
        r5 = new java.lang.Object[r5];
        r6 = 0;
        r5[r6] = r1;
        r1 = 1;
        r5[r1] = r2;
        r1 = 2;
        r5[r1] = r4;
        r1 = 3;
        r2 = r3.length();
        r2 = java.lang.Long.valueOf(r2);
        r5[r1] = r2;
        r1 = 4;
        r2 = c(r11);
        r5[r1] = r2;
        r0 = java.lang.String.format(r0, r5);
        goto L_0x0008;
    L_0x008d:
        r8 = android.text.TextUtils.isEmpty(r4);	 Catch:{ Exception -> 0x0176 }
        if (r8 != 0) goto L_0x00ed;
    L_0x0093:
        r8 = "* Built";
        r8 = r4.contains(r8);	 Catch:{ Exception -> 0x0176 }
        if (r8 == 0) goto L_0x00ba;
    L_0x009b:
        r8 = android.text.TextUtils.isEmpty(r1);	 Catch:{ Exception -> 0x0176 }
        if (r8 == 0) goto L_0x00ba;
    L_0x00a1:
        r1 = "MRaid";
        r8 = new java.lang.StringBuilder;	 Catch:{ Exception -> 0x0178 }
        r8.<init>();	 Catch:{ Exception -> 0x0178 }
        r9 = "MMJS BuildDate:";
        r8 = r8.append(r9);	 Catch:{ Exception -> 0x0178 }
        r8 = r8.append(r4);	 Catch:{ Exception -> 0x0178 }
        r8 = r8.toString();	 Catch:{ Exception -> 0x0178 }
        com.millennialmedia.android.MMLog.b(r1, r8);	 Catch:{ Exception -> 0x0178 }
        r1 = r4;
    L_0x00ba:
        r8 = "* Version";
        r8 = r4.contains(r8);	 Catch:{ Exception -> 0x0176 }
        if (r8 == 0) goto L_0x00e1;
    L_0x00c2:
        r8 = android.text.TextUtils.isEmpty(r0);	 Catch:{ Exception -> 0x0176 }
        if (r8 == 0) goto L_0x00e1;
    L_0x00c8:
        r0 = "MRaid";
        r8 = new java.lang.StringBuilder;	 Catch:{ Exception -> 0x0171 }
        r8.<init>();	 Catch:{ Exception -> 0x0171 }
        r9 = "MMJS Version:";
        r8 = r8.append(r9);	 Catch:{ Exception -> 0x0171 }
        r8 = r8.append(r4);	 Catch:{ Exception -> 0x0171 }
        r8 = r8.toString();	 Catch:{ Exception -> 0x0171 }
        com.millennialmedia.android.MMLog.b(r0, r8);	 Catch:{ Exception -> 0x0171 }
        r0 = r4;
    L_0x00e1:
        r4 = android.text.TextUtils.isEmpty(r0);	 Catch:{ Exception -> 0x0176 }
        if (r4 != 0) goto L_0x00ed;
    L_0x00e7:
        r4 = android.text.TextUtils.isEmpty(r1);	 Catch:{ Exception -> 0x0176 }
        if (r4 == 0) goto L_0x004e;
    L_0x00ed:
        r4 = r7 + 1;
        r7 = r4;
        goto L_0x0044;
    L_0x00f2:
        r0 = move-exception;
        r5 = r3;
        r3 = r6;
        r6 = r7;
        r10 = r0;
        r0 = r1;
        r1 = r4;
        r4 = r10;
    L_0x00fa:
        r7 = "MRaid";
        r8 = "MMJS stats exception: ";
        com.millennialmedia.android.MMLog.e(r7, r8, r4);	 Catch:{ all -> 0x0160 }
        if (r5 == 0) goto L_0x0106;
    L_0x0103:
        r5.close();	 Catch:{ Exception -> 0x010d }
    L_0x0106:
        if (r6 == 0) goto L_0x0058;
    L_0x0108:
        r6.close();	 Catch:{ Exception -> 0x010d }
        goto L_0x0058;
    L_0x010d:
        r4 = move-exception;
        r5 = "MRaid";
        r6 = "Exception closing file: ";
    L_0x0112:
        com.millennialmedia.android.MMLog.e(r5, r6, r4);
        goto L_0x0058;
    L_0x0117:
        r0 = move-exception;
        r5 = r3;
        r6 = r7;
    L_0x011a:
        if (r5 == 0) goto L_0x011f;
    L_0x011c:
        r5.close();	 Catch:{ Exception -> 0x014e }
    L_0x011f:
        if (r6 == 0) goto L_0x0124;
    L_0x0121:
        r6.close();	 Catch:{ Exception -> 0x014e }
    L_0x0124:
        throw r0;
    L_0x0125:
        r5 = "MMJS Stats: Version[%s] buildDate[%s] MD5[%s] dlDate[%s] Size[%d] Url[%s]";
        r6 = 6;
        r6 = new java.lang.Object[r6];
        r7 = 0;
        r6[r7] = r0;
        r0 = 1;
        r6[r0] = r1;
        r0 = 2;
        r6[r0] = r2;
        r0 = 3;
        r6[r0] = r4;
        r0 = 4;
        r1 = r3.length();
        r1 = java.lang.Long.valueOf(r1);
        r6[r0] = r1;
        r0 = 5;
        r1 = c(r11);
        r6[r0] = r1;
        r0 = java.lang.String.format(r5, r6);
        goto L_0x0008;
    L_0x014e:
        r1 = move-exception;
        r2 = "MRaid";
        r3 = "Exception closing file: ";
        com.millennialmedia.android.MMLog.e(r2, r3, r1);
        goto L_0x0124;
    L_0x0157:
        r4 = move-exception;
        r5 = "MRaid";
        r6 = "Exception closing file: ";
        goto L_0x0112;
    L_0x015d:
        r0 = move-exception;
        r6 = r7;
        goto L_0x011a;
    L_0x0160:
        r0 = move-exception;
        goto L_0x011a;
    L_0x0162:
        r0 = move-exception;
        r3 = r6;
        r6 = r7;
        r10 = r4;
        r4 = r0;
        r0 = r1;
        r1 = r10;
        goto L_0x00fa;
    L_0x016a:
        r0 = move-exception;
        r6 = r7;
        r10 = r1;
        r1 = r4;
        r4 = r0;
        r0 = r10;
        goto L_0x00fa;
    L_0x0171:
        r0 = move-exception;
        r10 = r0;
        r0 = r4;
        r4 = r10;
        goto L_0x00fa;
    L_0x0176:
        r4 = move-exception;
        goto L_0x00fa;
    L_0x0178:
        r1 = move-exception;
        r10 = r1;
        r1 = r4;
        r4 = r10;
        goto L_0x00fa;
        */
    }

    static void b(Context context, String str) {
        if (!TextUtils.isEmpty(str)) {
            ThreadUtils.a(new AnonymousClass_1(str, context));
        }
    }

    static String c(Context context) {
        return context != null ? context.getSharedPreferences("MillennialMediaSettings", 0).getString(h, AdTrackerConstants.BLANK) : AdTrackerConstants.BLANK;
    }

    static boolean c(Context context, String str) {
        if (TextUtils.isEmpty(str)) {
            return false;
        }
        Editor edit = context.getSharedPreferences("MillennialMediaSettings", 0).edit();
        edit.putString(h, str);
        edit.commit();
        return true;
    }

    static boolean d(Context context) {
        File e = e(context);
        if (e != null) {
            MMLog.b(k, "@@-MMJS File location: " + e.getAbsolutePath());
        }
        return e != null && e.exists();
    }

    static boolean d(Context context, String str) {
        boolean d = d(context);
        if (!d && l.equals(str) && e(context, str)) {
            MMLog.c(k, "Using bundled version of mm.js");
            return true;
        } else {
            MMLog.e(k, "Unable to save bundled version of mm.js");
        }
        String c = c(context);
        MMLog.b(k, "Previous mm.js url is <" + c + ">");
        return d && str.equals(c);
    }

    private static File e(Context context) {
        return new File(AdCache.h(context), e);
    }

    static boolean e(Context context, String str) {
        Object decode = Base64.decode(m);
        Object decode2 = Base64.decode(n);
        if (decode == null || decode2 == null) {
            return false;
        }
        Object obj = new Object[(decode.length + decode2.length)];
        System.arraycopy(decode, 0, obj, 0, decode.length);
        System.arraycopy(decode2, 0, obj, decode.length, decode2.length);
        return a(context, new ByteArrayInputStream(obj)) && c(context, str);
    }
}